...
public class ForwardTest {
    ...
    fit = glm(y~x1+x2+x3,family="gaussian")
    summary(fit)
    fit$fitted
     */
    @Test
    public void test_0010() {
        ...
        GLMProblem problem = new GLMProblem(
                ...
                true,
                new Gaussian());
        Forward instance = new Forward(problem, 0.05);
    ...
    y<-3*x1 + 5.2*x2 - 6.9*x3
    fit = glm(y~x1+x2+x3+x5+x6-1,family="gaussian")
    summary(fit)
    fit$fitted
     */
    @Test
    public void test_0020() {
        ...
        assertArrayEquals(new double[]{3, 5.2, -6.9, 0, 0},
                instance.getModel().beta.betaHat.toArray(), 1e-12);
        assertArrayEquals(new double[]{0, 0, 0, 0, 0},
                instance.getModel().beta.stderr.toArray(), 1e-12);
//        assertArrayEquals(new double[]{223740642172870.84375, 2204969953161323.25000, -1114086272254422.87500, 1.05021}), instance.getModel().beta.z, 1e-5));//invalid when stderr = 0
         ...
         */
        //assertEquals(-481.8370297265686, instance.getModel().AIC, 1e-15);
        assertArrayEquals(new int[]{1, 2, 3, 0, 5, 6}, instance.getFactors());
    }
    ...
    public void test_0030() {
        ...
        GLMProblem problem = new GLMProblem(
                ...
                true,
                new Binomial());
        Forward instance = new Forward(problem, 0.222);
