...
public class ExponentialDistributionTest {
    ...
    public void test_exponential_0010() {
        ...
        Double[] lambda1 = new Double[]{2., 1.};
        HiddenMarkovModel model1 = new HiddenMarkovModel(PI1, A1, new ExponentialDistribution(lambda1));
//        HiddenMarkovModel model1 = model; //using true parameters as initial estimates
        ...
        Matrix A2 = model2.A();
        Double[] lambda2 = ((ExponentialDistribution) model2.getDistribution()).getParams();

        assertTrue(AreMatrices.equal(A0, A2, 1e-1));
        for (int i = 0; i < lambda0.length; ++i) {
            assertEquals(1., lambda0[i] / lambda2[i], 2e-1);
        }
    }
}
